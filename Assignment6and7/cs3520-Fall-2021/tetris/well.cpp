...
// search for completed lines and remove them
int prune_well(well_t * w) {
    int lines = 0, filled;
    // loop through the well
    for(int i = w->upper_left_y; i < (w->upper_left_y + w->height); i++) {
      for(int j = w->upper_left_x + 1; j < (w->upper_left_x + w->width); j++) {
        if((mvinch(i, j) & A_CHARTEXT) != ' ') {
          filled ++;
        }
        if(filled > (w->width - 2)) {
          lines += 1;
          for(int y = i; y > (w->upper_left_y); y--) {
            for(int x = (w->upper_left_x + 1); x < (w->upper_left_x + w->width); x++) {
                mvprintw(y, x, "%c", (mvinch(y - 1, x) & A_CHARTEXT));
            }
          }
          filled = 0;
        }
